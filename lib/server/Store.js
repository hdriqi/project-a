...
class Store {
  ...
  async getMediaUsage() {
    const result = await this.client.db(this.projectName).collection('_media').find()
    const arr = await result.toArray()
    const allFilesize = arr.map(data => data.fileSize)
    const totalFileSize = allFilesize.reduce((a, b) => a + b, 0)
    ...
    return new Promise((resolve, reject) => {
      ...
      this.s3.putObject(params, async (perr, pres) => {
        if (perr) {
          ...
          resolve({
            url: `https://${self.bucketName}.s3-ap-southeast-1.amazonaws.com/${newName}`
          })
        }
      })
    })
  ...
  async deleteFile(key) {
    ...
    return new Promise((resolve, reject) => {
      self.s3.deleteObject(params, async (err, data) => {
        if (err) {
          console.log(err)
          reject(err)
        }
        else {
          await self.client.db(self.projectName).collection('_media').deleteOne({
            uniqueName: key
          })
          resolve(key)
        }
