...
const main = async ({ project, schemas }) => {
	...
	server.put('/api/collections/:collection/:id', async (req, res) => {
		...
		// update database
		const { value } = await store.client.db(project.name).collection(req.params.collection).findOneAndUpdate({
	...
	server.post('/api/collections/:collection', async (req, res) => {
		...
		// save to database
		await store.client.db(project.name).collection(String(req.params.id)).insertOne(input)
		// call after save
