...
const main = async ({ project, schemas }) => {
	...
	server.get('/api/collections/:id', async (req, res) => {
		try {
			const data = await store.get(project.name, req.params.id, req.query)
			return res.json({
				success: 1,
				data: data
			})
	...
	server.post('/api/collections/:id', async (req, res) => {
		...
		// save to database
		await store.client.db(project.name).collection(req.params.id).insertOne(input)
		// call after save
